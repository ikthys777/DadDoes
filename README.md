...
Family organizer PWA — clock, notes, recipes, meals & grocery list. Built for Scotty.

## Features

**Clock** — Three styles (digital, analog, minimal) with 14 color options. Wake lock keeps screen on.
...
Supabase real-time sync for grocery list (10s polling) and family recipe library (30s polling). Polling pauses on clock tab and when app is backgrounded to conserve battery. Family code scopes data between paired devices (Dad Does ↔ Mom Mode).
...
Dark background (#0a0a0a), green accent (#4ADE80).
