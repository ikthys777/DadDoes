# Dad Does — Claude Development Guide
...
- **State** — All React `useState`. No Redux, no context. LocalStorage via `store.get()`/`store.set()` with `dd-` prefix on all keys.
- **Color theme** — Dark (#0a0a0a background), green accent (#4ADE80). Settings uses #4ADE80 for toggles.
...
## Encrypted Backup
Export: AES-256-GCM encryption with user-provided family code as key. Exports all localStorage keys with `dd-` prefix plus API keys and Supabase credentials.
Import: Decrypts and restores all data.

## Service Worker
Cache name includes version number (`dad-does-v10`). Bump version on every deploy to bust cache. Strategy: network-first with cache fallback. On install, pre-caches all assets. On activate, deletes old caches.
...
4. If tab has data, add localStorage persistence with `dd-` prefix

### Adding a new icon
Search for `const IconName = mkIcon(` at the top. Add new icon using Lucide SVG path data.

### Updating service worker
Bump the version number in `sw.js`: `const CACHE_NAME = 'dad-does-vXX'`

## Paired App
Mom Mode (`MomMode` repo) is the companion app. Same codebase with purple (#BB86FC) theme, `mm-` localStorage prefix, and `MomMode` function name. All features are mirrored. Changes should be applied to both apps.
